...
The site's numbers are not typed into the page. `register/01_REGISTER.csv` is the register,
embedded and parsed at load, and every figure in the status bar is one of the counters in
`register/03_FORMULAS.md` computed from those rows. The same CSV pastes straight into a
Google Sheet, where the identical formulas produce the identical figures.

| File | What it is |
|---|---|
| `register/01_REGISTER.csv` | 20 requirements across 4 markets, with source, policy, SOP, evidence and provability columns |
| `register/02_REVIEW_CYCLES.csv` | Review interval per cycle type, so staleness is measured per row rather than by one global rule |
| `register/03_FORMULAS.md` | The computed columns and dashboard counters, as spreadsheet formulas |
