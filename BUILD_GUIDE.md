...
**What you are building:** a single-page, fourteen-tab operations console — a market
...
that computes which controls are unprovable, a provenance chain from statute to evidence,
a versioned register, an ownership map, a documentation-health view, and an interactive
change simulator that computes the blast radius of a regulatory change. Every identifier
in it is a link.
...
Any of those goes away the moment you add a build step. At ~1,700 lines it is near the
ceiling of what one file should carry, but the data is still the bulk of it — and the data that drives the newer
...
This panel is a design exercise, not a dashboard. The layout exists to carry one claim
per tab:
...
| End to end | Fifteen stages from a regulator publishing to being able to prove you complied |
| Provenance | A register that cannot trace to primary source cannot be defended |
| Register | Every requirement carries its instrument, or is flagged unsourced |
| Markets | The graph edges mean something specific, and it should be stated in words |
| Versions | A config change is a control change, and nothing is silently overwritten |
| Ownership | Two accountable departments is the same failure as none |
| Doc health | Document staleness is a different failure from register staleness |
...
<div class="wrap">
  ...
  <section class="view" id="e2e"></section>    <!-- Step 11 -->
  <section class="view" id="prov"></section>   <!-- Step 12 -->
  <section class="view" id="reg"></section>    <!-- Step 13 -->
  <section class="view" id="mkt"></section>    <!-- Step 14 -->
  <section class="view" id="ver"></section>    <!-- Step 15 -->
  <section class="view" id="own"></section>    <!-- Step 16 -->
  <section class="view" id="doc"></section>    <!-- Step 17 -->
  <section class="view" id="sim"></section>
  <section class="view" id="approach"></section>
  <footer></footer>
</div>
<script>/* Steps 8-19 */</script>
</body>
</html>
```
...
```html
<div class="strip">
<span><a data-go="mkt" href="#">MARKETS</a> <b>11</b> verified</span>
<span><a data-go="reg" href="#">REQUIREMENTS</a> <b>17</b></span>
<span><a data-go="anat" href="#">CONTROLS</a> <b>16</b></span>
<span><a data-go="anat:unprovable" href="#">UNPROVABLE</a> <b class="cr" id="upv">5</b></span>
<span><a data-go="prov:orphaned" href="#">ORPHANED</a> <b class="cr" id="orph">0</b></span>
<span><a data-go="prov:superseded" href="#">SUPERSEDED</a> <b class="cr" id="sups">0</b></span>
<span><a data-go="reg:gap" href="#">GAPS</a> <b class="cr">1</b></span>
<span><a data-go="doc:stale" href="#">STALE DOC</a> <b class="wn" id="sdoc">0</b></span>
</div>
```
...
**Every metric is a filter, not a decoration.** Each one carries a `data-go` that opens the
view it summarises — `GAPS 1` filters the register to that gap. A number you cannot click
through to is a number nobody can check.

**Include the bad numbers.** `GAPS`, `PARTIAL`, `STALE`, `UNPROVABLE`, `ORPHANED`,
`SUPERSEDED` are the credible part. A strip showing all green reads as a mock-up; a strip admitting that five
...
Fourteen buttons, fourteen sections, one class toggle. No framework — but by Step 17 the
tab switch becomes a one-line router, so extract it into `switchTab(v)` now rather than
letting the nav own a private copy of the logic.
...
## Step 11 — The end-to-end walkthrough

The view that makes the rest navigable. Fifteen stages, four lanes — external, automation,
human, record — from a regulator publishing something to being able to prove you complied.
Stage 15 loops back to stage 1, because this does not end, it cycles.

Draw it from data with a lane index and a row pitch:

```js
var LANEX = [80, 235, 390, 545], ROW = 62, TOP = 30, HALF = 70;
var x = LANEX[s.lane], y = TOP + i * ROW;
```

Everything else follows: the connector between stage *i* and *i+1* is a cubic whose control
points sit directly below and above the two boxes, so the line leaves and enters vertically
however far apart the lanes are.

**Four things worth getting right, all of which are easy to get wrong:**

1. **Put the detail card above the diagram, not below it.** The SVG is nearly a thousand
   units tall. A card underneath means clicking stage 3 scrolls the answer off the screen.
2. **Route the loop-back around the edge.** A cubic from the last stage to the first cuts a
   diagonal across all fifteen rows and reads as a connection between whatever it happens to
   pass through. An orthogonal path — out to `x=632`, up to `y=6`, back along the top —
   crosses nothing.
3. **Give the SVG its own marker.** `url(#ar2)` pointing at a marker defined in a *different*
   inline SVG does resolve in current browsers, but it is a dependency between two diagrams
   that nothing enforces. Define `#arflow` inside this SVG's own `<defs>`.
4. **Make the stages reachable without a mouse.** `tabindex="0"`, `role="button"` and an
   `aria-label` naming the stage and its lane, plus a `keydown` handler for Enter and Space.
   A click-only diagram is the most common accessibility failure in a data-driven page,
   because it never looks broken.

Keep the diagram label short and the detail long. Each stage carries a `s` (a subtitle that
must fit 140 units) and a `d` (the full explanation, shown in the card). Cramming
"Make.com scenario version bump; config diff logged" into a box makes it unreadable at
both sizes.

**Then link out.** Each stage carries a `go` triple — view, argument, label — rendered in
the card as "see the config history →". That single field is what turns the walkthrough
from an illustration into the index for everything else in the app: stage 4 opens the
register, stage 9 opens documentation health, stage 15 opens the controls that fail the
provable test.

**Autoplay needs a single stop path.** Every control that moves the stage — clicking a
stage, stepping, pressing play again — calls the same `stopPlay()`. Without that, a timer
keeps advancing underneath a user who has taken manual control.

---

## Step 12 — Provenance

The layer that makes the register defensible. Every control traces up to a primary source;
every source traces down to dated evidence.

```
EXTERNAL — we do not control these
  Statute → Regulator instrument → Licence condition   (held by the LICENSEE)
INTERNAL — we own everything below
  Requirement → Internal policy → SOP → Control → Evidence
```

Model sources as a self-referencing array — `parent_source_id` is what makes the chain
walkable — and give each one `superseded_by`, `verified` and a `url`:

```js
["S-KE-01","Statute","Gambling Control Act, 2025","KE","Parliament of Kenya",
 "Act No. 14 of 2025","2025-08-12","2025-08-26","", "https://…", 1, ""],
["S-KE-11","Instrument","Withholding tax — 20% on winnings excluding stake","KE",…,
 "S-KE-10", …]   // superseded_by points at its replacement
```

Walking up is then eight lines:

```js
function ancestry(sid){
  var out = [], s = sid ? SRCX[sid] : null, guard = 0;
  while(s && guard++ < 12){ out.unshift(s); s = s[11] ? SRCX[s[11]] : null; }
  return out;
}
```

The `guard` matters: a data-entry mistake that makes a source its own ancestor would
otherwise hang the page rather than show a bad row.

**Two alerts, both computed, both uncomfortable:**

```js
function orphanedCtls(){                     // no authority anywhere up the chain
  return Object.keys(CTLMETA).filter(function(c){
    var r = CTLMETA[c][0] ? REQX[CTLMETA[c][0]] : null;
    return !r || !r[3];
  });
}
function supersededCtls(){                   // enforcing a rule that no longer exists
  return Object.keys(CTLMETA).filter(function(c){
    return chainFor(c).srcs.some(function(s){ return !!s[8]; });
  });
}
```

An orphaned control is embarrassing — effort spent for no known reason. A superseded chain
is dangerous — a dead rule still being enforced. Both are invisible in a register that
does not model provenance, which is why most registers catch neither.

**Use a real example.** Kenya's Finance Act 2025 moved betting excise from 15% of the
stake to 5% of the amount deposited, and replaced the 20% withholding on winnings with 5%
on withdrawals. Point one register row at the old instrument and the alert has something
true to catch. A demo where every chain is intact demonstrates nothing.

**Label the tiers as prose, not as enum values.** `LICENCECONDITION` is a database
artifact leaking into the interface; map it through `TIERLBL` to "Licence condition"
before rendering.

---

## Step 13 — The register, and why it is a spreadsheet

The view every strip metric filters into — and the source of every figure on the page.

**Ship the register as a CSV and parse it at load.** Not a JS array typed to look like a
register: the actual file, embedded verbatim in a template literal, with the same file
committed beside the page so it can be pasted straight into a Google Sheet.

```js
var SHEET = parseCSV(REGISTER_CSV);          // 01_REGISTER.csv, unmodified
var CYCLE = {};                              // 02_REVIEW_CYCLES.csv
parseCSV(CYCLES_CSV).forEach(function(c){ CYCLE[c.review_cycle] = +c.max_days; });
```

That single decision is what lets the page claim its numbers are real. It also forces an
honesty the page would otherwise dodge: rows from markets nobody has checked carry an
empty `source_ref`, and render as `UNSOURCED` in red rather than as a blank cell. A blank
reads as "not filled in yet". An explicit flag reads as a finding.

**Compute the derived columns in the page exactly as the sheet computes them.** Each one is
a formula in `03_FORMULAS.md` and a function here, and they must agree:

```js
function colStale(r){
  if(!r.last_verified) return "NEVER";
  return days(r.last_verified, ASOF) > (CYCLE[r.review_cycle] || 9999) ? "STALE" : "";
}
function colProvable(r){ return (r.log_exists && r.actor_recorded && r.artifact_dated) ? "yes" : "NO"; }
function colOrphaned(r){ return r.source_ref ? "" : "ORPHANED"; }
```

Note `colStale` compares each row against **its own** review cycle rather than one global
rule. A licence condition and an advertising rule move at different speeds, and a register
that applies one interval to both is wrong in one direction for half its rows.

**Derive everything else from the register too** — control ownership from `owner_internal`,
control scope from `scope`, the control set itself from `control_id`. Anything you keep in
a parallel map will drift, and the drift is invisible until someone adds a row.

Filters are one variable and one re-render, not a library:

```js
var regFilter = "all";
function setRegFilter(f){
  regFilter = f;
  Array.prototype.forEach.call(document.querySelectorAll("#rfilt button"), function(b){
...
`aria-pressed` drives the styling, exactly as `aria-selected` does on the tabs. Always end
with the empty state — "Nothing matches this filter" — or a filter that legitimately
returns nothing looks like a bug.

---

## Step 14 — Market drill-down

The graph is a picture until the nodes are clickable. Add `data-go` straight onto the SVG
elements — the delegated handler in Step 16 does not care whether it is HTML or SVG:

```html
<circle class="nd" data-go="mkt:ke" style="cursor:pointer" role="link" tabindex="0"
        aria-label="ke market detail" cx="330" cy="105" r="9.5" fill="#efaa3c"/>
```

The drill-down carries market, in-force instruments, requirements, controls, health and
change history — but **the section that earns the view is shared dependencies, stated in
words**:

```js
function sharedWith(ctlId, code){
  return MKT.filter(function(m){ return m[0] !== code && m[7].indexOf(ctlId) > -1; })
            .map(function(m){ return m[1]; });
}
```

> 3 global controls shared with Ghana, Nigeria, Tanzania, Uganda and Benin. Changing any of
> them for Kenya changes them for those markets too.

That sentence is what the graph edges *mean*. A reader can infer it from the picture; they
should not have to. Unverified markets open the same view behind a banner — `Market
presence not independently verified — illustrative data only.`

---

## Step 15 — Version control

Everything below the licence-condition line is versioned: policies, SOPs, scenarios and
**configuration**. Config is the one people leave out, and it is the one that matters most
— a stake floor or an accepted-document list changes what the control does, so a config
change *is* a control change. Render it in the same history as policy or the story is
incomplete.

Five rules worth encoding in the data rather than the prose:

1. Draft → Approved → Live, and **nothing goes Live without an approver**. Render a missing
   approver in red rather than as an empty cell.
2. **Approval date and effective date are different dates.** Approved 12 August, effective
   26 August is normal; the gap must be visible, so compute and show it.
3. **Superseding is explicit.** A new version sets `superseded_by` on the old one. Nothing
   is silently overwritten.
4. `change_reason` is mandatory and links to the regulatory change that drove it — that
   link is what answers "why did this SOP change in August?"
5. `training_required` means the version is not fully live until completion is recorded.

The diff panel is **plain language, not a code diff**: "Withholding restated as 5% on
withdrawals, replacing 20% on winnings." A compliance officer has to be able to check the
summary against the requirement without reading a patch.

---

## Step 16 — Departments and RACI

Nine departments across three lines of defence, then the same data rendered twice.

**The RACI matrix** is the part that generates a finding, and the finding is computed:

```js
var as = row[1].filter(function(c){ return c === "A"; }).length;
if(as !== 1) tr.className = "bad";
```

Rows with **no A** and rows with **more than one A** are the same failure. Two accountable
departments means a creative ships because each assumed the other had signed it off. Flag
both in red and let the matrix accuse itself — never hand-shade the rows, or the shading
drifts from the data the first time someone edits a cell.

**Departmental load** counts controls per department and flags concentration above 40%,
because one department owning most of the controls is key-person risk wearing an org chart.

Then state the finding the view exists to produce: count `OursToEnable` requirements owned
by Product & platform. That number is the compliance dependency on the engineering
roadmap, and it is invisible until something fails.

---

## Step 17 — Documentation health

`STALE REG` counts register rows. Document staleness is a different failure with a
different owner, so split the strip metric rather than folding them together. Five panels,
worst first:

1. **Stale documents** — past their review cycle, sorted by days overdue.
2. **Orphaned documents** — SOPs linked to no control, policies linked to no requirement.
   Documents nobody uses that everybody maintains.
3. **Undocumented controls** — the inverse, and worse. People performing a control from
   memory.
4. **Process-doc drift** — a workflow version changed with no matching SOP bump. The single
   most common documentation failure, and almost nobody tracks it. This is the SCN-12
   output.
5. **Training gaps** — versions where training is required and completion is below 100%,
   naming who has not completed.

Fix the as-at date rather than using `Date.now()`:

```js
var ASOF = "2026-08-18";
function overdue(last, cycle){ return days(last, ASOF) - cycle; }
```

A design exercise whose numbers drift every time it is opened cannot be discussed. Print
the as-at date under the panels so the fixed clock is a stated assumption rather than a
bug.

---

## Step 18 — Interactivity: every ID is a link

The rule that makes it read as a system rather than a slide: **if it has an ID, it is
clickable.** One delegated listener handles the whole app, HTML and SVG alike:

```js
document.addEventListener("click", function(e){
  var t = e.target, a = null;
  while(t && t !== document){
    if(t.getAttribute && t.getAttribute("data-go")){ a = t; break; }
    t = t.parentNode;
  }
  if(!a) return;
  e.preventDefault();
  var raw = a.getAttribute("data-go"), i = raw.indexOf(":");
  go(i < 0 ? raw : raw.slice(0, i), i < 0 ? "" : raw.slice(i + 1));
});
```

Walk `parentNode` manually rather than using `closest()` — support is fine either way in
current browsers, but the loop works identically for SVG children, which is where half the
links live.

Then one router, one helper, and a per-view argument table:

```js
function go(v, arg){ switchTab(v); if(arg && ARG[v]) ARG[v](arg); }

function lnk(id){                                  // routes by prefix
  var v = id.indexOf("CTL-") === 0 ? "anat" : id.indexOf("REQ-") === 0 ? "reg"
        : id.indexOf("SCN-") === 0 ? "auto" : "ver";
  return "<a class='id' data-go='" + v + ":" + esc(id) + "' href='#'>" + esc(id) + "</a>";
}
```

`ARG` holds one function per view — expand this control, filter to that status, select that
market, preload that scenario. Adding a jump target is one entry, not a new mechanism.

**Two traps.** First, a link inside a click-to-expand row fires both handlers; put the link
in the expanded detail rather than the clickable row. Second, `switchTab` must be the
*only* place that flips `aria-selected` and `.on` — when I first wired this, the nav had
its own copy of that logic and the strip links quietly bypassed it.

---

## Step 19 — The change simulator
...
## Step 21 — Accessibility and motion
...
## Step 23 — Deploy to GitHub Pages
...
**Add a market's register rows.** Non-Kenyan markets are seeded from `REQTPL` by
`seedRegister()`, which takes the first N template rows per market and attaches that
market's own instrument as the source. Change `REQSEED` to change a market's count; the
strip, the drill-down and the register table all follow, because none of them holds its own
copy of the number. Seeded rows must stay visibly seeded — the register says so in prose
directly under the filters, and their sources render amber as `illustrative`.
...
**One register, and everything reads from it.** The hardest bug in this build was not a
crash: it was a second copy of the truth. A hand-written control map sat further down the
file than the map derived from the register, so `var` hoisting let the stale one win at
runtime and the provenance alerts quietly reported one orphaned control instead of four.
Two views also wrote their own totals into the status strip after `renderCounts()` had
already filled it, so the strip showed control counts under requirement labels.

The rule that fixes the whole class: **exactly one function writes the strip, and every
derived map is built from the register at load.** If a second definition exists, delete it
rather than keeping it in sync.

**Reconcile your numbers before shipping — by computing them, not by checking them.** The
strip, the panels and the drill-downs all state counts, and readers add them up. Two rounds
of this build shipped contradictions: `REQUIREMENTS 16` against a register health panel
implying 17, and later a strip claiming 16 requirements against per-market drill-downs that
summed to 72.

The durable fix was to delete every typed number from the strip. `MARKETS`, `REQUIREMENTS`,
`CONTROLS`, `GAPS`, `PARTIAL`, `STALE REG`, `STALE DOC`, `UNPROVABLE`, `ORPHANED` and
`SUPERSEDED` are all written by `renderCounts()` from the arrays at load, so they cannot
disagree with the views they summarise however the data changes.

Two consequences worth knowing. **Order matters**: every renderer has to run after all data
exists, including anything seeded at load — put the whole init block at the very end of the
script and keep it there, because a renderer that runs early silently produces a page built
from half the data. And **distinguish reported from modelled**: 11 markets have a publicly
reported presence but only 7 are modelled in the register, so the strip says both rather
than implying the other four have rows they do not have.
...
- [ ] Every requirement has a source, or renders as `UNSOURCED`
- [ ] Seeded rows are labelled as seeded, next to the rows that are genuinely sourced
- [ ] Register totals reconcile against the per-market drill-down counts
- [ ] The end-to-end diagram's loop-back crosses nothing
- [ ] Every stage is reachable by keyboard and links out to the view holding its detail
- [ ] Orphaned-control and superseded-chain counts are computed and shown
- [ ] At least one chain really is superseded, so the alert has something true to catch
- [ ] Every version shows approver, approval date, effective date and change reason
- [ ] Config changes appear in the same history as policy changes
- [ ] RACI flags rows with no A and rows with more than one A
- [ ] Every market node opens its drill-down
- [ ] The drill-down states shared dependencies in words, not just as edges
- [ ] Every status-strip metric filters into a view
- [ ] Every identifier rendered anywhere is a link
- [ ] Staleness is measured from a fixed as-at date, printed on the page
- [ ] Architecture holds five diagrams with prose between them
...
2. **Why training and effective-date tracking are stages, not admin.** A control that
   ...
   project plan.
3. **How an obligation travels from statute to evidence, and what breaks if a link is
   missing.** Statute → regulator instrument → licence condition → requirement → policy →
   SOP → control → evidence. Lose the top link and you cannot answer "show me your
   authority". Lose a middle one and you may be enforcing a rule that was superseded. Lose
   the bottom one and the control is unprovable, which in an audit is indistinguishable
   from absent.
4. **Why config changes are versioned like policy changes.** Because a config change is a
   control change: a stake floor or an accepted-document list changes what the control
   actually does. Versioning policy but not config means the register describes a control
   that no longer matches the system.
5. **What a RACI row with two accountable departments means in practice.** That nobody is
   accountable. Two A's is not redundancy — it is the state in which a creative ships
   because each department assumed the other had signed it off, and neither can be shown
   to have been wrong.
6. **How you would prove, six months later, that a given control operated.** Query the
